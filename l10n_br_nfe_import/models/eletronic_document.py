...
class EletronicDocument(models.Model):
    ...
    def get_ide(self, nfe, operacao):
        ...
        if dt_entrada_saida:
            ...
            dt_entrada_saida = dt_entrada_saida.astimezone(pytz.utc).replace(
                tzinfo=None
            )
    ...
    def get_transp(self, nfe):
        ...
        if hasattr(nfe.NFe.infNFe, "transp"):
            ...
            if transp.modFrete == 9:
                return dict(modalidade_frete=modFrete)
    ...
    def get_protNFe(self, nfe, company_id):
        ...
        if (
            protNFe.cStat in [100, 150]
            or protNFe.cStat == 110
            and company_id.l10n_br_cnpj_cpf in protNFe.chNFe
        ):
            return dict(
                chave_nfe=protNFe.chNFe,
                data_autorizacao=parser.parse(str(nfe.protNFe.infProt.dhRecbto)),
    ...
    def get_infAdic(self, nfe):
        info_adicionais = {
            "informacoes_legais": get(nfe, "NFe.infNFe.infAdic.infAdFisco"),
            "informacoes_complementares": get(nfe, "NFe.infNFe.infAdic.infCpl"),
        }
    ...
    def create_invoice_eletronic_item(
        self, item, company_id, partner_id, supplier, product_automation
    ):
        ...
        if seller_id:
            ...
            if len(product) > 1:
                message = "\n".join(
                    [
                        "Produto: %s - %s" % (x.default_code or "", x.name)
                        for x in product
                    ]
                )
                raise UserError(
                    "Existem produtos duplicados com mesma codificação, corrija-os antes de prosseguir:\n%s"
                    % message
                )

        if not product and item.prod.cEAN and str(item.prod.cEAN) != "SEM GTIN":
            product = self.env["product.product"].search(
                [("barcode", "=", item.prod.cEAN)], limit=1
            )

        uom_id = (
            self.env["uom.uom"].search([("name", "=", str(item.prod.uCom))], limit=1).id
        )
        ...
        invoice_eletronic_Item = {
            ...
            "unidade_medida": str(item.prod.uCom),
            "cfop": cfop,
            "ncm": ncm,
            "product_ean": item.prod.cEAN,
            "product_cprod": codigo,
            "product_xprod": item.prod.xProd,
            "cest": cest,
            "item_pedido_compra": nItemPed,
        ...
        return self.env["eletronic.document.line"].create(invoice_eletronic_Item)

    def _get_icms(self, imposto):
        csts = ["00", "10", "20", "30", "40", "41", "50", "51", "60", "70", "90"]
        csts += ["101", "102", "103", "201", "202", "203", "300", "400", "500", "900"]
        ...
        for cst in csts:
            ...
            if tag_icms:
                ...
                vals = {
                    "icms_cst": cst_item,
                    "origem": get(icms, "%s%s.orig" % (tag_icms, cst), str),
                    "icms_tipo_base": get(icms, "%s%s.modBC" % (tag_icms, cst), str),
                    "icms_aliquota_diferimento": get(
                        icms, "%s%s.pDif" % (tag_icms, cst)
                    ),
                    "icms_valor_diferido": get(icms, "%s%s.vICMSDif" % (tag_icms, cst)),
                    ...
                    "icms_base_calculo": get(icms, "%s%s.vBC" % (tag_icms, cst)),
                    "icms_aliquota_reducao_base": get(
                        icms, "%s%s.pRedBC" % (tag_icms, cst)
                    ),
                    "icms_aliquota": get(icms, "%s%s.pICMS" % (tag_icms, cst)),
                    "icms_valor": get(icms, "%s%s.vICMS" % (tag_icms, cst)),
                    ...
                    "icms_st_aliquota_mva": get(icms, "%s%s.pMVAST" % (tag_icms, cst)),
                    "icms_st_base_calculo": get(icms, "%s%s.vBCST" % (tag_icms, cst)),
                    "icms_st_aliquota_reducao_base": get(
                        icms, "%s%s.pRedBCST" % (tag_icms, cst)
                    ),
                    "icms_st_aliquota": get(icms, "%s%s.pICMSST" % (tag_icms, cst)),
                    "icms_st_valor": get(icms, "%s%s.vICMSST" % (tag_icms, cst)),
                    "icms_bc_uf_dest": get(imposto, "ICMSUFDest.vBCUFDest"),
                    "icms_aliquota_fcp_uf_dest": get(imposto, "ICMSUFDest.pFCPUFDest"),
                    "icms_aliquota_uf_dest": get(imposto, "ICMSUFDest.pICMSUFDest"),
                    ...
                    "icms_fcp_uf_dest": get(imposto, "ICMSUFDest.vFCPUFDest"),
                    "icms_uf_dest": get(imposto, "ICMSUFDest.vICMSUFDest"),
                    "icms_uf_remet": get(imposto, "ICMSUFDest.vICMSUFRemet"),
                }
    ...
    def _get_di(self, di):
        ...
        state_id = self.env["res.country.state"].search(
            [("code", "=", state_code), ("country_id.code", "=", "BR")]
        )
        vals = {
            "name": get(di, "nDI"),
    ...
    def get_items(self, nfe, company_id, partner_id, supplier, product_automation):
        items = []
    ...
    def import_nfe(
        ...
        company_id = self.env["res.company"].browse(partner_vals["company_id"])
        ...
        invoice_dict.update(self.get_ide(nfe, partner_vals["tipo_operacao"]))
        ...
        invoice_dict.update({"reboque_ids": [(0, None, self.get_reboque(nfe))]})
    ...
    def _create_partner(self, tag_nfe, destinatary):
        ...
        partner_id.message_post(
            body="<ul><li>Parceiro criado através da importação\
                                de xml</li></ul>"
        )
    ...
    def _create_product(self, company_id, supplier, nfe_item, uom_id=False):
        ...
        seq_id = int(params.get_param("br_nfe_import.product_sequence", default=0))
        if not seq_id:
            raise UserError(
                "A empresa não possui uma sequência de produto configurado!"
            )
        ncm = get(nfe_item, "NCM", str)
        ncm_id = self.env["account.ncm"].search([("code", "=", ncm)])
    ...
    def _create_tax(self, tax_domain, aliquota, company_id):
        ...
        message = (
            "<ul><li>Aliquota criada através da importação\
                   do xml da nf %s<br/></li></ul>"
            % self.numero
        )

        return tax, message

    def _create_supplierinfo(self, item, purchase_order_line, automation=False):
        ...
        if not supplierinfo_id:
            ...
            message = (
                "<ul><li>Produto do fornecedor criado através da\
                ...
                % {
                    "nf": self.numero,
                    "codigo_produto_fornecedor": item.product_cprod,
                    "descricao_produto_fornecedor": item.product_xprod,
                    "codigo_produto": purchase_order_line.product_id.default_code,
                    "descricao_produto": purchase_order_line.product_id.name,
                }
            )
    ...
    def _get_purchase_line_id(
        ...
        purchase_line_id = purchase_line_ids[int(item.item_pedido_compra) - 1]
        ...
        if hasattr(purchase_line_id.product_id, "seller_id"):
            ...
            if seller_id and seller_id.product_code == item.product_cprod:
                ...
                return purchase_line_ids.filtered(
                    lambda x: x.product_id.seller_id.product_code == item.product_cprod
                )
    ...
    def _get_company_invoice(self, nfe, partner_automation):
        ...
        company = (
            ...
            .sudo()
            .search([("partner_id.l10n_br_cnpj_cpf", "=", dest_cnpj_cpf)])
        )
        ...
        if not company:
            company = (
                self.env["res.company"]
                .sudo()
                .search([("partner_id.l10n_br_cnpj_cpf", "=", emit_cnpj_cpf)])
            )
            if company:
    ...
    def generate_eletronic_document(self, xml_nfe, create_partner):
        ...
        invoice_dict.update(self.get_protNFe(nfe, company_id))
        invoice_dict.update(self.get_main(nfe))
        partner = self.get_partner_nfe(nfe, partner_vals["destinatary"], create_partner)
        invoice_dict.update(self.get_ide(nfe, partner_vals["tipo_operacao"]))
        invoice_dict.update(partner)
        invoice_dict.update(self.get_ICMSTot(nfe))
        invoice_dict.update(
    ...
    def _prepare_account_invoice_vals(self):
        operation = "in_invoice" if self.tipo_operacao == "entrada" else "out_invoice"
        journal_id = (
        ...
        account_id = (
            ...
            if operation == "in_invoice"
            else partner.property_account_receivable_id.id
        )
        ...
        vals = {
            ...
            "invoice_payment_term_id": self.env.ref(
                "l10n_br_nfe_import.payment_term_for_import"
            ).id,
        }
        return vals
