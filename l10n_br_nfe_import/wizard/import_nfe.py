...
class WizardImportNfe(models.TransientModel):
    ...
    import_batch_zip = fields.Boolean(
        string="Importar Zip?",
        help="Se marcado esta opção é possível \
    ...
    zip_file_error_name = fields.Char(default="XMLs não importados.zip", readonly=True)
    skip_wrong_xml = fields.Boolean(
        string="Ignorar Xml com erro?",
        help="Se marcado vai ignorar os xmls \
    ...
    def action_import_nfe(self):
        ...
        if self.import_batch_zip:
            ...
            xml_list.append({"name": "NF-e", "file": base64.b64decode(self.nfe_xml)})
