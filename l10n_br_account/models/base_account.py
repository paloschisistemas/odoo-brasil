...
class AccountServiceType(models.Model):
    ...
    parent_id = fields.Many2one("account.service.type", "Tipo de Serviço Pai")
    child_ids = fields.One2many(
        "account.service.type", "parent_id", "Tipo de Serviço Filhos"
    )
...
class AccountNcm(models.Model):
    ...
    def notify_account_users(self, message):
        ...
        for user in users:
            ...
            _, odoobot_id = self.env["ir.model.data"]._xmlid_to_res_model_res_id(
                "base.partner_root"
            )

            channel_info = self.env["mail.channel"].channel_get(
                [partner.id, odoobot_id], pin=True
            )
    ...
    def cron_sync_average_tax_rate(self):
        if not self.env.company.l10n_br_ibpt_api_token:
            message = (
                "Você ainda não configurou o sistema para a lei de Olho no Imposto. "
