...
class ResCompany(models.Model):
    ...
    l10n_br_cnpj_cpf = fields.Char(
        string="CNPJ", compute="_compute_address", inverse="_inverse_cnpj_cpf"
    )
    l10n_br_legal_name = fields.Char(
        string="Razão Social", compute="_compute_address", inverse="_inverse_legal_name"
    )
    l10n_br_district = fields.Char(
        string="Bairro", compute="_compute_address", inverse="_inverse_district"
    )
    l10n_br_number = fields.Char(
        string="Número", compute="_compute_address", inverse="_inverse_number"
    )
    ...
    l10n_br_suframa = fields.Char(
        string="Suframa", compute="_compute_address", inverse="_inverse_suframa"
    )
