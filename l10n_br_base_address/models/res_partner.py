...
class ResPartner(models.Model):
    ...
    _sql_constraints = [
        (
            "res_partner_l10n_br_cnpj_cpf_uniq",
            "unique (l10n_br_cnpj_cpf)",
    ...
    def action_check_sefaz(self):
        if self.l10n_br_cnpj_cpf and self.state_id:
            ...
            resposta = consulta_cadastro(
                certificado, obj=obj, ambiente=1, estado=self.state_id.l10n_br_ibge_code
            )
            ...
            if info.cStat == 111 or info.cStat == 112:
                ...
                if not city and xMun:
                    city = self.env["res.city"].search(
                        [("name", "ilike", xMun), ("state_id", "=", self.state_id.id)]
                    )
                if city:
