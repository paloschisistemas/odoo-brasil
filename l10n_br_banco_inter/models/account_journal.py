...
class AccountJournal(models.Model):
    ...
    def sync_bank_statement_online_inter(self, start=None, end=None):
        overlap = self.env["account.bank.statement"].search_count(
            [("date", ">=", start), ("date", "<=", end)]
        )
        overlap2 = self.env["account.bank.statement"].search_count(
            [("end_date", ">=", start), ("end_date", "<=", end)]
        )
        overlap3 = self.env["account.bank.statement"].search_count(
        ...
        statement = self.env["account.bank.statement"].create(
            {
                "name": "Extrato de {0} até {1}".format(
            ...
            self.env["account.bank.statement.line"].create(
                {
                    ...
                    "amount": float(titulo["valor"])
                    if titulo["tipoOperacao"] == "C"
                    else (float(titulo["valor"]) * -1.0),
                }
            )
