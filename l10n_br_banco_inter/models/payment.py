...
class PaymentProvider(models.Model):
    ...
    code = fields.Selection(
        selection_add=[("boleto-inter", "Boleto Banco Inter")],
        ondelete={"boleto-inter": "set default"},
    )
...
class PaymentTransaction(models.Model):
    ...
    def action_get_pdf_inter(self):
        ...
        for transaction in self:
            if transaction.provider_id.code != "boleto-inter":
                continue

            journal_id = self.provider_id.journal_id
            boleto_data = self.get_boleto_inter_pdf(
                journal_id, transaction.provider_reference
            )
    ...
    def action_verify_transaction(self):
        if self.provider_id.code != "boleto-inter":
        ...
        journal_id = self.provider_id.journal_id
        situacao = self.get_boleto_inter_status(journal_id, self.provider_reference)
    ...
    def action_cancel_transaction(self):
        ...
        self._set_transaction_cancel()
        journal_id = self.provider_id.journal_id
        self.cancel_boleto_inter(journal_id, self.provider_reference)
