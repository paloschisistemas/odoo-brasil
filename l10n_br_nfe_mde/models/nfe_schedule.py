...
class NfeSchedule(models.TransientModel):
    ...
    def _mask_cnpj_cpf(cnpj_cpf):
        ...
        if len(val) == 11:
            return "%s.%s.%s-%s" % (val[0:3], val[3:6], val[6:9], val[9:11])
        else:
            return "%s.%s.%s/%s-%s" % (
                val[0:2],
                val[2:5],
    ...
    def schedule_download(self, raise_error=False):
                        ...
                        if nfe["schema"] == "resNFe_v1.01.xsd":
                            ...
                            if obj_nfe:
                                ...
                                manifesto = {
                                    ...
                                    "partner_id": partner.id,
                                    "data_emissao": datetime.strptime(
                                        str(infNfe.ide.dhEmi)[:19], "%Y-%m-%dT%H:%M:%S"
                                    ),
                                    "company_id": company.id,
                                    "forma_inclusao": "Verificação agendada",
                                    "nfe_processada": base64.encodestring(nfe["xml"]),
                                    "nfe_processada_name": "NFe%08d.xml"
                                    % infNfe.ide.nNF,
                                }
        ...
        if raise_error:
            raise UserError(
                "Total de documentos localizados %s\n%s" % (total, "\n".join(messages))
            )
    ...
    def cron_manifest_automation(self):
        ...
        for company in companies:
            ...
            manifestos = self.env["nfe.mde"].search(
                [("company_id", "=", company.id), ("is_processed", "=", False)],
                limit=50,
            )
