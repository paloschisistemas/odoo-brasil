...
class NfeMde(models.Model):
    ...
    company_id = fields.Many2one(
        "res.company", string="Empresa", default=_default_company, readonly=True
    )
    ...
    numero_sequencial = fields.Char(string="Número Sequencial", readonly=True, size=20)
    ...
    tipo_operacao = fields.Selection(
        [("0", "Entrada"), ("1", "Saída")], string="Tipo de Operação", readonly=True
    )
    ...
    state = fields.Selection(
        string="Situação da Manifestação",
        readonly=True,
        selection=[
            ("pending", "Pendente"),
    ...
    nfe_processada_name = fields.Char(string="Nome Xml da NFe", size=100, readonly=True)
    ...
    def _check_partner_id(self):
        if self.partner_id and self.cnpj_fornecedor != self.partner_id.l10n_br_cnpj_cpf:
    ...
    def _create_event(self, code, message, mde_id):
        return {"code": code, "name": message, "nfe_mde_id": mde_id}
    ...
    def action_known_emission(self):
        ...
        nfe_result = send_event(
            self.company_id, self.chave_nfe, "ciencia_operacao", self.id, evento=evento
        )
    ...
    def action_confirm_operation(self):
        ...
        nfe_result = send_event(
            self.company_id, self.chave_nfe, "confirma_operacao", self.id, evento=evento
        )
    ...
    def action_unknown_operation(self):
        ...
        if nfe_result["code"] == 135:
            ...
            self.state = "desconhecido"
            self.message_post(
                body="Desconhecimento da operação já previamente realizado"
            )
    ...
    def action_not_operation(self, context=None, justificativa=None):
        ...
        if not justificativa:
            return {
                ...
                "view_mode": "form",
                "target": "new",
                "context": {"default_mde_id": self.id},
            }
        ...
        if nfe_result["code"] == 135:
            ...
            self.state = "nao_realizado"
            self.message_post(
                body="Tentativa de Operação não realizada ja previamente realizada"
            )
    ...
    def action_import_xml(self):
        for item in self:
            ...
            invoice_eletronic.import_nfe(
                ...
                nfe,
                nfe_xml,
                company.partner_automation,
                company.invoice_automation,
                company.tax_automation,
                company.supplierinfo_automation,
                invoice_dict=vals,
            )
    ...
    def action_test_bot(self):
        self.send_message_to_user(
            self.env.user, "Olá usuário, chegou uma nova nova fiscal!"
        )
