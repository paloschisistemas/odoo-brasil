...
class AccountTax(models.Model):
    ...
    def _compute_ipi(self, price_base, sign):
        ...
        if (
            "ipi_base_calculo_manual" in self.env.context
            and self.env.context["ipi_base_calculo_manual"] > 0
        ):
    ...
    def _compute_icms_st(self, price_base, ipi_value, icms_value, sign):
        ...
        if (
            "icms_st_base_calculo_manual" in self.env.context
            and self.env.context["icms_st_base_calculo_manual"] > 0
        ):
            base_icmsst = self.env.context["icms_st_base_calculo_manual"]
        if icmsst_tax.icms_st_incluso:
            icmsst = round(
                (
                    (base_icmsst - icms_value)
                    * (icmsst_tax.amount / 100.0)
                    / (1 - icmsst_tax.amount / 100.0)
                )
                - icms_value,
                2,
            )
        ...
        if fcpst_tax:
            ...
            fcpst_vals["amount"] = fcpst_tax.l10n_br_compute_amount_tax(
                base_icmsst, sign
            )
    ...
    def _compute_difal(self, price_base, ipi_value, sign):
        ...
        for tax_inter in icms_inter:
            ...
            vals_inter["amount"] = round(
                (interno - interestadual) * sign * (100 - icms_inter_part) / 100, 2
            )
            vals_intra["amount"] = round(
                (interno - interestadual) * sign * icms_inter_part / 100, 2
            )
    ...
    def _compute_pis_cofins(self, price_base, sign):
        ...
        for tax in pis_cofins_tax:
            ...
            if tax.domain == "pis":
                if (
                    "pis_base_calculo_manual" in self.env.context
                    and self.env.context["pis_base_calculo_manual"] > 0
                ):
            ...
            if tax.domain == "cofins":
                if (
                    "cofins_base_calculo_manual" in self.env.context
                    and self.env.context["cofins_base_calculo_manual"] > 0
                ):
                    vals["amount"] = tax.l10n_br_compute_amount_tax(
                        self.env.context["cofins_base_calculo_manual"], 1.0
                    )
                    vals["base"] = self.env.context["cofins_base_calculo_manual"]
                else:
                    vals["amount"] = tax.l10n_br_compute_amount_tax(price_base, sign)
                    vals["base"] = price_base
    ...
    def _compute_ii(self, price_base, sign):
        ...
        if not ii_tax:
            return []
        if (
            "ii_base_calculo" in self.env.context
            and self.env.context["ii_base_calculo"] > 0
        ):
    ...
    def _compute_retention(self, price_base, sign):
        retention_tax = self.filtered(lambda x: x.domain in ("csll", "irrf", "inss"))
    ...
    def sum_taxes(self, price_base, sign):
        ...
        icms = self._compute_icms(price_base, ipi[0]["amount"] if ipi else 0.0, sign)
        ...
        difal = self._compute_difal(price_base, ipi[0]["amount"] if ipi else 0.0, sign)
