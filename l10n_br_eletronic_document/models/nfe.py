...
class EletronicDocument(models.Model):
    ...
    def _compute_format_danfe_key(self):
        for item in self:
            item.chave_nfe_danfe = re.sub(
                "(.{4})", "\\1.", item.chave_nfe, 10, re.DOTALL
            )
    ...
    def _hook_validation(self):
        ...
        if (
            not self.company_id.partner_id.zip
        ...
        for eletr in self.document_line_ids:
            ...
            if not eletr.product_id.default_code:
                errors.append("Prod: %s - Código do produto" % (eletr.product_id.name))
        ...
        if self.model == "nfe":
            ...
            if (
                self.company_id.l10n_br_accountant_id
                and not self.company_id.l10n_br_accountant_id.l10n_br_cnpj_cpf
            ):
    ...
    def _prepare_eletronic_invoice_item(self, item, invoice):
        ...
        if self.ambiente != "homologacao":
            xProd = item.product_id.with_context(display_default_code=False).name_get()[
                0
            ][1]
        else:
 ...
 SEM VALOR FISCAL"
        ...
        qty_precis = self.env["decimal.precision"].precision_get(
            ...
            imposto.update(
                {
                    "ISSQN": {
                        ...
                        "cMunFG": "%s%s"
                        % (
                            invoice.company_id.state_id.l10n_br_ibge_code,
 ...
 SEM VALOR FISCAL"
        ...
        for item in self.document_line_ids:
            eletronic_items.append(self._prepare_eletronic_invoice_item(item, self))
        total = {
            # ICMS
            ...
            "vFCPSTRet": "0.00",
            "vProd": "%.02f" % sum(self.document_line_ids.mapped("valor_bruto")),
        ...
        if self.valor_servicos > 0.0:
            issqn_total = {
                ...
                "vDeducao": "",
                "vOutro": "",
                "vISSRet": "%.02f" % self.iss_valor_retencao
                if self.iss_valor_retencao
                else "",
            }
            tributos_retidos = {
                "vRetPIS": "%.02f" % self.iss_valor_retencao
                if self.iss_valor_retencao
                else "",
                "vRetCOFINS": "%.02f" % self.cofins_valor_retencao
                if self.cofins_valor_retencao
                else "",
                "vRetCSLL": "%.02f" % self.csll_valor_retencao
                if self.csll_valor_retencao
                else "",
                "vBCIRRF": "%.02f" % self.irrf_base_calculo
                if self.irrf_valor_retencao
                else "",
                "vIRRF": "%.02f" % self.irrf_valor_retencao
                if self.irrf_valor_retencao
                else "",
                "vBCRetPrev": "%.02f" % self.inss_base_calculo
                if self.inss_valor_retencao
                else "",
                "vRetPrev": "%.02f" % self.inss_valor_retencao
                if self.inss_valor_retencao
                else "",
            }
        if self.transportadora_id.street:
            end_transp = "%s - %s, %s" % (
                ...
                self.transportadora_id.l10n_br_district or "",
            )
        else:
            end_transp = ""
        transp = {
            ...
            "transporta": {
                "xNome": self.transportadora_id.l10n_br_legal_name
                or self.transportadora_id.name
                or "",
                "IE": re.sub(
                    "[^0-9]", "", self.transportadora_id.l10n_br_inscr_est or ""
                ),
                "xEnder": end_transp if self.transportadora_id else "",
            ...
            volumes.append(
                {
                    ...
                    "pesoL": "%.03f" % item.peso_liquido if item.peso_liquido else "",
        ...
        cobr = {
            "fat": {
                ...
                "vOrig": "%.02f" % (self.fatura_liquido + self.fatura_desconto),
        ...
        if len(duplicatas) > 0 and self.fiscal_position_id.finalidade_emissao not in (
            "2",
            "4",
        ):
            vals["cobr"] = cobr
    ...
    def action_send_eletronic_invoice(self):
        ...
        if resposta_recibo:
            ...
            self._create_attachment("rec-ret", self, resposta_recibo["received_xml"])
        ...
        _logger.info(
            "NF-e (%s) was finished with status %s" % (self.numero, self.codigo_retorno)
        )
    ...
    def action_cancel_document(self, context=None, justificativa=None):
        ...
        if not justificativa:
            return {
                ...
                "view_mode": "form",
                "target": "new",
                "context": {"default_edoc_id": self.id},
            }
        ...
        if resposta.cStat == 128 and resposta.retEvento.infEvento.cStat in (
            135,
            136,
            155,
        ):
            ...
            return self._create_response_cancel(code, motive, resp, justificativa)
        ...
        _logger.info(
            "Cancelling NF-e (%s) was finished with status %s"
    ...
    def action_get_status(self):
        ...
        if retorno_consulta.cStat == 101:
            ...
            message = "%s - %s" % (retorno_consulta.cStat, retorno_consulta.xMotivo)
    ...
    def _get_hash_csrt(self):
        ...
        hash_csrt = base64.b64encode(hashlib.sha1(hash_csrt.encode()).digest())
