...
class AccountMove(models.Model):
    ...
    def _compute_nfe_number(self):
        for item in self:
            docs = self.env["eletronic.document"].search([("move_id", "=", item.id)])
            if docs:
    ...
    def _validate_for_eletronic_document(self):
        ...
        for move in self:
            ...
            if (
                not move.company_id.partner_id.zip
                or len(re.sub(r"\D", "", self.company_id.partner_id.zip)) != 8
            ):
            ...
            if not move.company_id.partner_id.state_id:
                ...
                if not move.company_id.partner_id.state_id.l10n_br_ibge_code:
                    errors.append(
                        "Cadastro da Empresa / Endereço - Cód. do IBGE do estado"
                    )
            ...
            if not move.company_id.partner_id.city_id:
                ...
                if not move.company_id.partner_id.city_id.l10n_br_ibge_code:
                    errors.append(
                        "Cadastro da Empresa/Endereço - Cód. do IBGE do município"
                    )
            ...
            if not move.company_id.partner_id.country_id:
                ...
                if not move.company_id.partner_id.country_id.l10n_br_ibge_code:
                    errors.append(
                        "Cadastro da Empresa / Endereço - Código do BC do país"
                    )
            ...
            for eletr in doc_lines:
                ...
                if eletr.product_id.type in ("consu", "product"):
                    has_products = True
                prod = "Produto: %s - %s" % (
                    eletr.product_id.default_code,
                    eletr.product_id.name,
                )
                if not eletr.product_id.default_code:
                    errors.append(
                        "Prod: %s - Código do produto" % (eletr.product_id.name)
                    )
                ...
                if (
                    move.company_id.l10n_br_accountant_id
                    and not move.company_id.l10n_br_accountant_id.l10n_br_cnpj_cpf
                ):
                    errors.append(
                        "Cadastro da Empresa / CNPJ do escritório contabilidade"
                    )
            ...
            if has_services:
                ...
                if cod_municipio == "4205407":
                    if not all(
                        ...
                        errors.append(
                            "Campos de validação para a API de Florianópolis não estão preenchidos"
                        )
            ...
            if partner.country_id.id == company.partner_id.country_id.id:
                if not partner.state_id:
                    ...
                    if not partner.state_id.l10n_br_ibge_code:
                        errors.append(
                            "Cliente / Endereço - Código do IBGE \
                                    do estado"
                        )
    ...
    def _prepare_eletronic_doc_vals(self, invoice_lines, numero_nfe):
        ...
        vals["cod_regime_tributario"] = (
            "1" if invoice.company_id.l10n_br_tax_regime == "simples" else "3"
        )
        ...
        if invoice.commercial_partner_id.is_company:
            if invoice.commercial_partner_id.l10n_br_inscr_est:
                ind_ie_dest = "1"
            elif invoice.commercial_partner_id.state_id.code in (
                "AM",
                "BA",
                "CE",
                "GO",
        ...
        duplicatas = []
        count = 1
        for parcela in invoice.receivable_move_line_ids.sorted(
        ...
        for inv_line in invoice_lines:
            total_desconto += round(
                inv_line.price_unit * inv_line.quantity * inv_line.discount / 100, 2
            )
    ...
    def sum_line_taxes(self, vals):
        ...
        return {
            ...
            "valor_icms_uf_remet": sum(
                line[2].get("icms_uf_remet", 0) for line in lines
            ),
            "valor_icms_fcp_uf_dest": sum(
                line[2].get("icms_fcp_uf_dest", 0) for line in lines
            ),
            ...
            "valor_bc_icmsst": sum(
                line[2].get("icms_st_base_calculo", 0) for line in lines
            ),
            "pis_valor_retencao": sum(
                line[2].get("pis_valor_retencao", 0) for line in lines
            ),
            "cofins_valor_retencao": sum(
                line[2].get("cofins_valor_retencao", 0) for line in lines
            ),
            "irrf_base_calculo": sum(
                line[2].get("irrf_base_calculo", 0) for line in lines
            ),
            "irrf_valor_retencao": sum(
                line[2].get("irrf_valor_retencao", 0) for line in lines
            ),
            "csll_base_calculo": sum(
                line[2].get("csll_base_calculo", 0) for line in lines
            ),
            "csll_valor_retencao": sum(
                line[2].get("csll_valor_retencao", 0) for line in lines
            ),
            "inss_base_calculo": sum(
                line[2].get("inss_base_calculo", 0) for line in lines
            ),
            "inss_valor_retencao": sum(
                line[2].get("inss_valor_retencao", 0) for line in lines
            ),
        }
    ...
    def action_post(self):
        moves = self.filtered(
            lambda x: (
                ...
                or x.l10n_br_edoc_policy == "after_payment"
            )
    ...
    def get_eletronic_line_vals(self):
        ...
        vals = {
            ...
            "icms_valor_credito": round(
                self.price_total * fiscal_pos.icms_aliquota_credito / 100, 2
            ),
            # - IPI -
            "ipi_cst": "99",
        ...
        if self.move_id.move_type in ["in_invoice", "out_refund"]:
            if (
                self.move_id.company_id.state_id
                == self.move_id.commercial_partner_id.state_id
            ):
                cfop = "1" + cfop[1:]
            else:
                cfop = "2" + cfop[1:]
