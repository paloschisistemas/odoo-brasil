...
class EletronicDocument(models.Model):
    ...
    partner_shipping_id = fields.Many2one("res.partner", string="Entrega")
    ...
    # ------------ Retencoes ------------
    irrf_base_calculo = fields.Monetary(string="Base IRRF")
    irrf_valor_retencao = fields.Monetary(string="Valor IRRF")
    inss_base_calculo = fields.Monetary(string="Base INSS")
    inss_valor_retencao = fields.Monetary(string="Valor INSS")

    valor_produtos = fields.Monetary(string="Valor Produtos")
    valor_servicos = fields.Monetary(string="Valor Serviços")
    ...
    schedule_user_id = fields.Many2one(
        "res.users", string="Agendado por", readonly=True, tracking=True
    )
    tipo_operacao = fields.Selection(
        [("entrada", "Entrada"), ("saida", "Saída")], string="Tipo de Operação"
    )
    ...
    numero = fields.Integer(string="Número", copy=False)
    numero_rps = fields.Integer(string="Número RPS", copy=False)
    numero_controle = fields.Integer(string="Número de Controle", copy=False)
    data_agendada = fields.Date(
        string="Data agendada",
        default=fields.Date.today,
    )
    data_emissao = fields.Datetime(string="Data emissão", copy=False)
    data_autorizacao = fields.Char(string="Data de autorização", size=30, copy=False)
    ambiente = fields.Selection(
        [("homologacao", "Homologação"), ("producao", "Produção")], string="Ambiente"
    )
    ...
    payment_term_id = fields.Many2one(
        ...
        string="Condição pagamento",
    )
    fiscal_position_id = fields.Many2one(
        "account.fiscal.position",
        string="Posição Fiscal",
    )
    natureza_operacao = fields.Char(string="Natureza da Operação", size=100)
    ...
    valor_bc_icms = fields.Monetary(string="Base ICMS")
    ...
    valor_estimado_tributos = fields.Monetary(
        string="Tributos Estimados", compute="_compute_valor_estimado_tributos"
    )
    ...
    codigo_retorno = fields.Char(string="Código Retorno", tracking=True, copy=False)
    mensagem_retorno = fields.Char(string="Mensagem Retorno", tracking=True, copy=False)
    ...
    salvar_xml_enviado = fields.Boolean(string="Salvar Xml Enviado?", default=False)
    ...
    ind_ie_dest = fields.Selection(
        ...
        help="Indicador da IE do desinatário",
    )
    ...
    placa_veiculo = fields.Char(string="Placa do Veículo", size=7)
    ...
    reboque_ids = fields.One2many(
        "nfe.reboque", "eletronic_document_id", string="Reboques"
    )
    volume_ids = fields.One2many(
        "nfe.volume", "eletronic_document_id", string="Volumes"
    )
    ...
    local_embarque = fields.Char(string="Local de Embarque", size=60)
    local_despacho = fields.Char(string="Local de Despacho", size=60)

    # Cobrança
    numero_fatura = fields.Char(string="No. Fatura")
    ...
    duplicata_ids = fields.One2many(
        "nfe.duplicata", "eletronic_document_id", string="Duplicatas"
    )
    ...
    sequencial_evento = fields.Integer(
        string="Sequêncial Evento", default=1, copy=False
    )
    recibo_nfe = fields.Char(string="Recibo NFe", size=50, copy=False)
    chave_nfe = fields.Char(string="Chave NFe", size=50, copy=False)
    chave_nfe_danfe = fields.Char(
        string="Chave Formatado", compute="_compute_format_danfe_key"
    )
    protocolo_nfe = fields.Char(
        string="Protocolo", size=50, help="Protocolo de autorização da NFe", copy=False
    )
    nfe_processada = fields.Binary(string="Xml NFe", readonly=True, copy=False)
    nfe_processada_name = fields.Char(
        string="Nome do XML NFe", size=100, readonly=True, copy=False
    )

    nfse_url = fields.Char(string="URL da NFe", size=500, readonly=True, copy=False)
    nfse_pdf = fields.Binary(string="PDF da NFe", readonly=True, copy=False)
    ...
    related_document_ids = fields.One2many(
        "nfe.related.document",
        "eletronic_document_id",
        "Documentos Fiscais Relacionados",
    )

    # CARTA DE CORRECAO
    cartas_correcao_ids = fields.One2many(
        "carta.correcao.eletronica.evento",
        "eletronic_document_id",
    ...
    cert_state = fields.Selection(
        related="company_id.l10n_br_cert_state", readonly=True
    )
    ...
    def _compute_legal_information(self):
        fiscal_ids = self.fiscal_position_id.fiscal_observation_ids.filtered(
            ...
            and x.tipo_produto
            in [False] + self.document_line_ids.mapped("tipo_produto")
        )
    ...
    def _get_variables_msg(self):
        return {"user": self.env.user, "ctx": self._context, "invoice": self.move_id}
    ...
    def unlink(self):
        for item in self:
            if not item.can_unlink():
                raise UserError(_("Documento Eletrônico enviado - Proibido excluir"))
        ...
        self.env["mail.activity"].create(
            {
                ...
                "res_model_id": self.env.ref(
                    "l10n_br_eletronic_document.model_eletronic_document"
                ).id,
            }
        )
    ...
    def _get_nfes_to_send(self, limit):
        ...
        nfes = inv_obj.search(
            [("state", "in", states), ("data_agendada", "<=", fields.Date.today())],
            limit=limit,
        )
    ...
    def cron_send_nfe(self, limit=50):
        ...
        for item in nfes:
            try:
                _logger.info(
                    "Sending edoc id: %s (number: %s) by cron" % (item.id, item.numero)
                )
                item.action_send_eletronic_invoice()
            except Exception as e:
                item.log_exception(e)
                item.notify_user()
                _logger.error("Erro no envio de documento eletrônico", exc_info=True)
            finally:
                self.env.cr.commit()
    ...
    def _find_attachment_ids_email(self):
        ...
        if self.model == "nfse":
            ...
            if danfe_city:
                danfe_name = (
                    "l10n_br_eletronic_document.main_template_br_nfse_%s" % danfe_city
                )
    ...
    def send_email_nfe(self):
        ...
        _logger.info(
            "Sending e-mail for e-doc %s (number: %s)" % (self.id, self.numero)
        )
    ...
    def generate_dict_values(self):
        ...
        for doc in self:
            ...
            data = {
                ...
                "data_emissao_hora": doc.data_emissao.astimezone(tz).strftime(
                    "%Y-%m-%dT%H:%M:%S"
                ),
    ...
    def action_cancel_document(self, context=None, justificativa=None):
        ...
        doc_values = {
            ...
            "codigo_municipio": "%s%s"
            % (company.state_id.l10n_br_ibge_code, company.city_id.l10n_br_ibge_code),
        }
        if doc_values["codigo_municipio"] == "4205407":
            ...
            doc_values["numero"] = str(self.data_emissao.year) + "{:>011d}".format(
                self.numero
            )
            response = cancel_api(certificate, password, doc_values)
        else:
        ...
        if response["code"] in (200, 201):
            ...
            if response.get("xml", False):
                ...
                split_nfe_processada = base64.decodebytes(self.nfe_processada).split(
                    b"</Nfse>"
                )
    ...
    def qrcode_floripa_url(self):
        ...
        urlconsulta = (
            "http://nfps-e.pmf.sc.gov.br/consulta-frontend/#!/\
consulta?cod=%s&cmc=%s"
            % (self.protocolo_nfe, self.company_id.l10n_br_inscr_mun)
        )

        url = (
            '<img class="center-block"\
style="max-width:100px;height:100px;margin:0px 0px;"src="/report/barcode/\
?type=QR&width=100&height=100&value='
            + urllib.parse.quote(urlconsulta)
            + '"/>'
        )
        return url
...
class EletronicDocumentLine(models.Model):
    ...
    name = fields.Char(string="Name")
    eletronic_document_id = fields.Many2one("eletronic.document", string="Documento")
    ...
    state = fields.Selection(related="eletronic_document_id.state", string="State")

    product_id = fields.Many2one("product.product", string="Produto")
    tipo_produto = fields.Selection(
        [("product", "Produto"), ("service", "Serviço")], string="Tipo Produto"
    )
    cfop = fields.Char("CFOP", size=5)
    ncm = fields.Char("NCM", size=10)
    unidade_medida = fields.Char("Un. Medida Xml", size=10)

    item_lista_servico = fields.Char(string="Código do serviço", size=10)
    codigo_servico_municipio = fields.Char(string="Código NFSe", size=20)
    descricao_codigo_municipio = fields.Char(
        string="Descrição Código Serviço", size=100
    )
    # Florianopolis
    codigo_cnae = fields.Char(
        string="CNAE",
        size=10,
    )
    # Paulistana
    ...
    uom_id = fields.Many2one("uom.uom", string="Unidade Medida")
    quantidade = fields.Float(string="Quantidade", digits="Product Unit of Measure")
    preco_unitario = fields.Monetary(string="Preço Unitário")
    ...
    item_pedido_compra = fields.Char(
        string="Item de compra", size=20, help="Item do pedido de compra do cliente"
    )

    frete = fields.Monetary(string="Frete")
    seguro = fields.Monetary(string="Seguro")
    desconto = fields.Monetary(string="Desconto")
    outras_despesas = fields.Monetary(string="Outras despesas")

    def _compute_tributos_estimados(self):
        for item in self:
            tributos_estimados = 0.0
            ncm = (
                item.product_id.service_type_id
            ...
            if ncm:
                ...
                if item.product_id.l10n_br_origin in ["0", "3", "4", "5", "8"]:
                    ncm_mult = (
                        ncm.federal_nacional
                        + ncm.estadual_imposto
                        + ncm.municipal_imposto
                    ) / 100
                else:
                    ncm_mult = (
                        ncm.federal_importado
                        + ncm.estadual_imposto
                        + ncm.municipal_imposto
                    ) / 100
    ...
    tributos_estimados = fields.Monetary(
        string="Valor Estimado Tributos", compute="_compute_tributos_estimados"
    )
    ...
    origem = fields.Selection(ORIGEM_PROD, string="Origem Mercadoria", default="0")
    icms_cst = fields.Selection(
        CST_ICMS + CSOSN_SIMPLES,
        string="CST ICMS",
    )
    ...
    icms_base_calculo = fields.Monetary(string="Base ICMS")
    icms_aliquota_reducao_base = fields.Float(
        string="% Redução Base ICMS",
        digits="Account",
    )
    icms_valor = fields.Monetary(string="Valor ICMS")
    icms_valor_credito = fields.Monetary(string="Valor de Cŕedito")
    ...
    icms_st_base_calculo = fields.Monetary(string="Base ICMS ST")
    icms_st_aliquota_reducao_base = fields.Float(
        string="% Redução Base ST",
        digits="Account",
    )
    icms_st_valor = fields.Monetary(string="Valor ICMS ST")

    fcp_st_aliquota = fields.Float(
        string="FCP ST Alíquota",
        digits="Account",
    )
    fcp_st_valor = fields.Monetary(string="Valor FCP ST")

    icms_valor_original_operacao = fields.Float(
        string="ICMS da Operação",
        digits="Account",
    )
    ...
    icms_motivo_desoneracao = fields.Char(string="Motivo Desoneração", size=2)
    ...
    ipi_base_calculo = fields.Monetary(string="Base IPI")
    ipi_reducao_bc = fields.Float(
        string="% Redução Base",
        digits="Account",
    )
    ipi_valor = fields.Monetary(string="Valor IPI")
    ...
    pis_base_calculo = fields.Monetary(string="Base PIS")
    pis_valor = fields.Monetary(string="Valor PIS")
    pis_valor_retencao = fields.Monetary(string="Retenção PIS")
    ...
    cofins_base_calculo = fields.Monetary(string="Base COFINS")
    cofins_valor = fields.Monetary(string="Valor COFINS")
    cofins_valor_retencao = fields.Monetary(string="Retenção COFINS")
    ...
    iss_base_calculo = fields.Monetary(string="Base ISS")
    iss_valor = fields.Monetary(string="Valor ISS")
    iss_valor_retencao = fields.Monetary(string="Valor Retenção")

    # ------------ RETENÇÔES ------------
    csll_base_calculo = fields.Monetary(string="Base CSLL")
    csll_aliquota = fields.Float(
        string="Alíquota CSLL",
        digits="Account",
    )
    csll_valor = fields.Monetary(string="Valor CSLL")
    csll_valor_retencao = fields.Monetary(string="Retenção CSLL")
    irpj_base_calculo = fields.Monetary(string="Base IRPJ")
    irpj_aliquota = fields.Float(
        string="Alíquota IRPJ",
        digits="Account",
    )
    irpj_valor = fields.Monetary(string="Valor IRPJ")
    irpj_valor_retencao = fields.Monetary(string="Retenção IRPJ")
    irrf_base_calculo = fields.Monetary(string="Base IRRF")
    irrf_aliquota = fields.Float(
        string="Alíquota IRRF",
        digits="Account",
    )
    irrf_valor = fields.Monetary(string="Valor IRRF")
    irrf_valor_retencao = fields.Monetary(string="Retenção IRRF")
    inss_base_calculo = fields.Monetary(string="Base INSS")
    inss_aliquota = fields.Float(
        string="Alíquota INSS",
        digits="Account",
    )
    inss_valor = fields.Monetary(string="Valor INSS")
    inss_valor_retencao = fields.Monetary(string="Retenção INSS")
    ...
    cest = fields.Char(
        string="CEST", size=10, help="Código Especificador da Substituição Tributária"
    )
    codigo_beneficio = fields.Char(string="Benefício Fiscal", size=10)
    extipi = fields.Char(
        string="Código EX TIPI",
        size=3,
    )
    classe_enquadramento_ipi = fields.Char(string="Classe Enquadramento", size=5)
    codigo_enquadramento_ipi = fields.Char(
        string="Código Enquadramento",
        size=3,
        default="999",
    )
    ...
    import_declaration_ids = fields.One2many(
        ...
        "eletronic_document_line_id",
        string="Declaração de Importação",
    )

    # ----------- ICMS INTERESTADUAL -----------
    tem_difal = fields.Boolean(string="Difal?")
    icms_bc_uf_dest = fields.Monetary(string="Base ICMS Difal")
    icms_aliquota_fcp_uf_dest = fields.Float(string="% FCP")
    icms_aliquota_uf_dest = fields.Float(string="% ICMS destino")
    icms_aliquota_interestadual = fields.Float(string="% ICMS Inter")
    icms_aliquota_inter_part = fields.Float(string="% Partilha", default=100.0)
    icms_uf_remet = fields.Monetary(string="ICMS Remetente")
    icms_uf_dest = fields.Monetary(string="ICMS Destino")
    icms_fcp_uf_dest = fields.Monetary(string="Valor FCP")
    informacao_adicional = fields.Text(string="Informação Adicional")

    # =========================================================================
    # ICMS Retido anteriormente por ST
    # =========================================================================
    icms_substituto = fields.Monetary("ICMS Substituto")
    icms_bc_st_retido = fields.Monetary("Base Calc. ST Ret.")
    icms_aliquota_st_retido = fields.Float("% ST Retido")
    icms_st_retido = fields.Monetary("ICMS ST Ret.")
