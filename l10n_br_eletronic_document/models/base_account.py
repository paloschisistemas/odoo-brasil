...
class NfeCFOP(models.Model):
    ...
    type = fields.Selection(
        [("input", "Entrada"), ("output", "Saída")], "Tipo", required=True
    )
    parent_id = fields.Many2one("nfe.cfop", "CFOP Pai")
    child_ids = fields.One2many("nfe.cfop", "parent_id", "CFOP Filhos")
    ...
    _sql_constraints = [
        ("nfe_cfop_code_uniq", "unique (code)", "Já existe um CFOP com esse código !")
    ]
...
class AccountCNAE(models.Model):
    ...
    child_ids = fields.One2many("account.cnae", "parent_id", "CNAEs Filhos")
    internal_type = fields.Selection(
        [("view", "Visualização"), ("normal", "Normal")],
        "Tipo Interno",
        required=True,
...
class ImportDeclaration(models.Model):
    ...
    move_id = fields.Many2one("account.move", "Fatura", ondelete="cascade", index=True)
    eletronic_document_line_id = fields.Many2one(
        "eletronic.document.line",
        "Linha de Documento Eletrônico",
        ondelete="cascade",
        index=True,
    )
    ...
    afrmm_value = fields.Float("Valor da AFRMM", digits="Account", default=0.00)
    ...
    line_ids = fields.One2many(
        "nfe.import.declaration.line", "import_declaration_id", "Linhas da DI"
    )
...
class NfeRelatedDocumento(models.Model):
    ...
    state_id = fields.Many2one(
        "res.country.state", "Estado", domain="[('country_id.code', '=', 'BR')]"
    )
    cnpj_cpf = fields.Char("CNPJ/CPF", size=18)
    cpfcnpj_type = fields.Selection(
        [("cpf", "CPF"), ("cnpj", "CNPJ")], "Tipo Doc.", default="cnpj"
    )
    ...
    def _check_ie(self):
        ...
        if self.inscr_est:
            ...
            try:
                mod = __import__(
                    "odoo.addons.br_base.tools.fiscal", globals(), locals(), "fiscal"
                )
...
class NfeFiscalObservation(models.Model):
    ...
    tipo = fields.Selection(
        [("fiscal", "Observação Fiscal"), ("observacao", "Observação")], string="Tipo"
    )
